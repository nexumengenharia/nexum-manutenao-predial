...
/* ------------------------------------------------------------------
   ...
   ISOLAMENTO ENTRE TRIBUNAIS
   Estas consultas foram escritas confiando apenas na RLS. Só que a
   aplicacao conecta como dona das tabelas, e o Postgres dispensa o dono
   das policies enquanto FORCE ROW LEVEL SECURITY estiver desligado — o
   resultado era o painel de um tribunal somando as ordens do outro.
   Agora todo FROM carrega `tenant_id = manutencao.tenant_atual()`, a
   mesma expressao da policy, lida do app.tenant_id que comContexto()
   grava na transacao. O filtro passa a ser explicito e verificavel, e
   continua correto no dia em que a RLS for forcada.
------------------------------------------------------------------- */

export async function resumoExecutivo(ctx: Contexto) {
  return consultarUm(ctx, `
    with base as (
      select * from manutencao.ordem
    ...
    select j.*,
      ...
      (select count(*) from manutencao.solicitacao
        where excluido_em is null and tenant_id = manutencao.tenant_atual()
          and situacao in ('ABERTA','TRIAGEM'))                                      as solicitacoes_fila,
      (select count(*) from manutencao.solicitacao
        where excluido_em is null and tenant_id = manutencao.tenant_atual()
          and situacao in ('ABERTA','TRIAGEM','EM_EXECUCAO')
          and prazo_em < now())                                                      as solicitacoes_vencidas,
      (select count(*) from manutencao.controle
        where excluido_em is null and tenant_id = manutencao.tenant_atual()
          and situacao = 'VENCIDO')                                                  as controles_vencidos,
      (select count(*) from manutencao.controle
        where excluido_em is null and tenant_id = manutencao.tenant_atual()
          and situacao = 'A_VENCER')                                                 as controles_a_vencer,
      (select count(*) from manutencao.item_estoque
        where excluido_em is null and tenant_id = manutencao.tenant_atual()
          and quantidade <= quantidade_minima)                                       as estoque_critico,
      (select count(*) from manutencao.contratada
        where excluido_em is null and tenant_id = manutencao.tenant_atual()
          and contrato_fim between current_date and current_date + 90)               as contratos_vencendo,
      (select count(*) from manutencao.ativo
        where excluido_em is null and tenant_id = manutencao.tenant_atual()
          and situacao = 'PARADO')                                                   as ativos_parados,
      (select count(*) from manutencao.veiculo
        where excluido_em is null and tenant_id = manutencao.tenant_atual())         as veiculos,
      (select count(*) from manutencao.veiculo
        where excluido_em is null and tenant_id = manutencao.tenant_atual()
          and situacao = 'EM_MANUTENCAO')                                            as veiculos_manutencao,
...
export const indicadorPredio = (ctx: Contexto) => consultar(ctx, `
  select * from manutencao.vw_indicador_predio
   where tenant_id = manutencao.tenant_atual()
   order by custo_total desc`);

export const indicadorTipo = (ctx: Contexto) => consultar(ctx, `
  select * from manutencao.vw_indicador_tipo
   where tenant_id = manutencao.tenant_atual()
...
export const vencimentosProximos = (ctx: Contexto, n = 10) => consultar(ctx, `
  select c.id, c.nome, c.tipo, c.norma, c.proxima_data, c.situacao, c.custo_previsto,
   ...
   where c.excluido_em is null and c.tenant_id = manutencao.tenant_atual()
     and c.situacao in ('VENCIDO','A_VENCER')
...
export const quadroAtividades = (ctx: Contexto) => consultar(ctx, `
  select s.id, s.numero, s.titulo, s.situacao, s.prioridade, s.natureza,
   ...
   where s.excluido_em is null and s.tenant_id = manutencao.tenant_atual()
     and s.situacao in ('ABERTA','TRIAGEM','EM_EXECUCAO')
...
export const cargaPorEquipe = (ctx: Contexto) => consultar(ctx, `
  select e.id, e.nome, e.natureza, e.sla_horas,
    ...
    left join manutencao.solicitacao s
      on s.equipe_id = e.id and s.excluido_em is null
     and s.tenant_id = manutencao.tenant_atual()
   where e.excluido_em is null and e.ativo
     and e.tenant_id = manutencao.tenant_atual()
